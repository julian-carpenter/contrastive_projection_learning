...
class Model(tf.keras.models.Model):
    ...
    def __init__(self, num_classes, **kwargs):
        ...
        self.hidden_model = resnet.resnet(
            resnet_depth=FLAGS.resnet_depth,
            width_multiplier=FLAGS.width_multiplier,
            cifar_stem=FLAGS.image_size <= 32)
        # Augs on GPU
        _augmentation = {"brightness": 0.5, "jitter": 0.2, "fill_value": 0,
                         "scale": (0.2, 1.0), "fill_scale": (0.05, 0.3),
                         "fill_ratio": (0.25, 2.5), "strength": 0.25,
                         "ratio": (1., 1.)}

        img_shape = [FLAGS.image_size, FLAGS.image_size, 1]
        self.aug_gpu = data_util.gpu_augmenter(in_shape=img_shape,
                                               **_augmentation)
        ...
        self.final_out = tf.keras.layers.Activation('linear', dtype='float32')
    ...
    def __call__(self, inputs, training):
        ...
        num_transforms = inputs.shape[-1]
        # num_transforms = tf.repeat(1, num_transforms)
        # Split channels, and optionally apply extra batched augmentation.
        features_list = tf.split(
            features, num_or_size_splits=num_transforms, axis=-1)
        features = tf.concat(features_list, 0)  # (num_transforms * bsz, h, w, c)
        # Augs on GPU
        features = self.aug_gpu(features, training=training)
        # Base network forward pass.
        hiddens = self.hidden_model(features, training=training)

        # Add heads.
        projection_head_outputs, supervised_head_inputs = self._projection_head(hiddens, training)
        projection_head_outputs = self.final_out(projection_head_outputs)
        supervised_head_inputs = self.final_out(supervised_head_inputs)

        if FLAGS.train_mode == 'finetune':
            supervised_head_outputs = self.supervised_head(supervised_head_inputs, training)
            supervised_head_outputs = self.final_out(supervised_head_outputs)
            return None, supervised_head_outputs, None
            ...
            supervised_head_outputs = self.final_out(supervised_head_outputs)
            return projection_head_outputs, supervised_head_outputs, supervised_head_inputs
        else:
            return projection_head_outputs, None, None
